...
public class ConcertDAO {
    ...
    public boolean addConcert(String concertName, Date concertDate, double regularPrice, double vipPrice) {
        String sql = "INSERT INTO concerts (concert_name, concert_date, regular_price, vip_price) VALUES (?, ?, ?, ?)";
        try (Connection conn = db.getConnection(); PreparedStatement pstmt = conn.prepareStatement(sql)) {
            pstmt.setString(1, concertName);
    ...
    public boolean updateConcert(int id, String concertName, Date concertDate, double regularPrice, double vipPrice) {
        String sql = "UPDATE concerts SET concert_name=?, concert_date=?, regular_price=?, vip_price=? WHERE id=?";
        try (Connection conn = db.getConnection(); PreparedStatement pstmt = conn.prepareStatement(sql)) {
            pstmt.setString(1, concertName);
            pstmt.setDate(2, concertDate);
            pstmt.setDouble(3, regularPrice);
