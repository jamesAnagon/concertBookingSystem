...
public class BookingDAO {
    ...
    public boolean createBooking(String customerName, int concertId, String concertName, String seatType, int quantity, double price) {
        ...
        String updateSeatsSql = seatType.equalsIgnoreCase("VIP") ?
                "UPDATE concerts SET vip_seats = vip_seats - ? WHERE id = ? AND vip_seats >= ?" :
                "UPDATE concerts SET basic_seats = basic_seats - ? WHERE id = ? AND basic_seats >= ?";
        try (Connection conn = db.getConnection()) {
            conn.setAutoCommit(false);
            try (PreparedStatement upd = conn.prepareStatement(updateSeatsSql)) {
                upd.setInt(1, quantity);
                upd.setInt(2, concertId);
            ...
            try (PreparedStatement ins = conn.prepareStatement(insertSql)) {
                ...
                ins.setDouble(6, price * quantity);
                ins.executeUpdate();
            }
            conn.commit();
            return true;
    ...
    public List<Map<String, Object>> getAllBookings() {
        String sql = "SELECT b.*, c.date AS concert_date FROM bookings b LEFT JOIN concerts c ON b.concert_id = c.id";
