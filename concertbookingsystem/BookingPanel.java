    ...
    private JPanel buildCreateCard() {
        ...
        if (concerts.isEmpty()) {
            ...
            for (int i = 0; i < concerts.size(); i++) {
                concertTitles[i] = String.valueOf(concerts.get(i).getOrDefault("concert_name", "Untitled"));
            }
        }
        ...
        concertChoice.setEnabled(!concerts.isEmpty());
        p.add(concertChoice, c);

        c.gridx = 0; c.gridy = 3; c.gridwidth = 2;
        JLabel noConcertsNotice = new JLabel("No concerts available at the moment.", SwingConstants.CENTER);
        noConcertsNotice.setForeground(new Color(110, 110, 110));
        noConcertsNotice.setFont(new Font("SansSerif", Font.ITALIC, 13));
        noConcertsNotice.setVisible(concerts.isEmpty());
        p.add(noConcertsNotice, c);
        ...
        createBtn.setEnabled(!concerts.isEmpty());
        createBtn.addActionListener(e -> {
            ...
            for (java.util.Map<String, Object> citem : concerts) {
                if (selectedConcert.equals(String.valueOf(citem.getOrDefault("concert_name", "")))) {
                    concert = citem;
                    break;
                }
            }
            ...
            double unitPrice = idx == 0 ? regularPrice : vipPrice;
            boolean ok = bookingDAO.createBooking(name, concertId, selectedConcert + " (auto)", idx == 0 ? "BASIC" : "VIP", 1, unitPrice);
