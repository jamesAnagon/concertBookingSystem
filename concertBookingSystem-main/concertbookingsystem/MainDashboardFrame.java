...
public class MainDashboardFrame extends JFrame {
    ...
    private void showEventDropdown(Component invoker) {
        if (concertsList.isEmpty()) {
            JOptionPane.showMessageDialog(this, "No concerts are available at the moment.", "No Concerts", JOptionPane.INFORMATION_MESSAGE);
            return;
        }

        JPopupMenu popup = new JPopupMenu();
        for (java.util.Map<String, Object> c : concertsList) {
            String title = String.valueOf(c.getOrDefault("concert_name", "Untitled"));
            Object d = c.get("concert_date");
            String dateStr = "";
            try {
    ...
    private JPanel buildHomeCard() {
        ...
        if (concertsList.isEmpty()) {
            JLabel noneLabel = new JLabel("No concerts at the moment.", SwingConstants.CENTER);
            noneLabel.setFont(new Font("SansSerif", Font.BOLD, 22));
            noneLabel.setForeground(PRIMARY_COLOR.darker());
            JPanel emptyPanel = new JPanel(new BorderLayout());
            emptyPanel.setBackground(OFF_WHITE);
            emptyPanel.add(noneLabel, BorderLayout.CENTER);
            p.add(emptyPanel, BorderLayout.CENTER);
            return p;
        }
        ...
        for (int i = 0; i < concertsList.size(); i++) {
            ...
            eventNamesArr[i] = String.valueOf(c.getOrDefault("concert_name", "Untitled"));
            Object d = c.get("concert_date");
            String formatted = "";
            try {
                if (d instanceof java.sql.Date) {
